...
| 2026-09-25 | Implementação E1-S3 | Implementar taxa base mensal por moeda e vigência, com seeds demonstrativos e consulta interna | Migration V3, `BaseRateQuery`, adapter JPA e provas de domínio/PostgreSQL sem endpoint ou escrita pública | A revisão removeu uma segunda porta pública intermediária e corrigiu tradução de falhas de persistência, fidelidade da origem, assinatura tipada e alcance do guardrail arquitetural. O agente não executou commits nem merge; posteriormente, a autora realizou os commits definitivos `7b097c9`, `9154e0b`, `bc9f3ee`, `cc2ff9d`, `3e691f7`, `cf9ac9d`, `431d50a` e `ab1519c` e integrou o PR à `main` por Rebase and merge. |
| 2026-09-25 | Implementação E2-S1 | Implementar catálogo interno e resolução de spread por Strategy, sem fórmula ou endpoint | Migration V4, duas Strategies `BigDecimal`, registry extensível, catálogo JPA, erros estáveis, log/métrica e testes de domínio/PostgreSQL/arquitetura | A decisão humana manteve spreads no código e apenas `strategy_key` no banco; a implementação conteve falhas transacionais e preservou E2-S2/E2-S3 fora do escopo. O agente não executou commits nem merge. Posteriormente, a autora realizou os commits definitivos `796687c`, `b828536`, `9374fd0`, `ccf1ebb`, `4a2e289`, `45d2dd3` e `1af402e`, obteve aprovação dos jobs `backend`, `frontend` e `repository` e integrou o PR à `main` por Rebase and merge. |
| 2026-09-26 | Implementação E2-S2 | Implementar simulação determinística na moeda do título conforme a story aprovada | Endpoint sem persistência, calendário ANBIMA versionado 2025–2030, `big-math` 2.3.2 isolado, V5, metadata de moeda, métrica/log e testes unitários/integração/arquitetura | ArchUnit detectou referências diretas `pricing→CurrencyCode` e `api→domain`; o overload público experimentado foi removido, `CurrencyMetadata` passou a preservar o `CurrencyCode` canônico e as exceções foram traduzidas no service. O agente não executou commits. Posteriormente, a autora realizou manualmente `e26c0cc feat(db): add currency minor-unit metadata`, `a5ab6e2 feat(pricing): simulate present value in title currency`, `c721f73 test(pricing): prove title-currency simulation contracts` e `1939fcd docs(pricing): document title-currency simulation`. |
